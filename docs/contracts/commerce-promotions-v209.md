...
| `POST /admin/coupons/{couponId}/delete` | 删除草稿/停用券；`clientRequestId`、`expectedVersion` 必填；启用券须先停用 |
| `POST /merchant/products/{productId}/delete` | 商品发布权限；删除同时停止销售，保留订单与退款；相同请求字段 |
...
确认状态按 `(couponId, owner_uuid)` 唯一保存：`viewed=false` 只表示已展示顶部提醒，`viewed=true` 同时表示已提醒与已查看，响应为 `data.acknowledged=true`。状态只前进，不回退，重复请求和两个设备并发确认安全。确认不减少券额度、不占用券、不触发交易；确认仅接受自己的受众范围，不公开其他玩家记录。券在展示与确认之间过期、被停用或使用，仍可保留原提醒确认；未开始活动不提前确认。

App 在启动完成或回到前台后读取未查看券，统一使用既有应用内顶部通知卡片，不再弹出模态优惠窗。其他浮层、键盘或通知忙碌时延后，不占用卡片外的页面触摸区域。单券和多券都可以点击查看“我的优惠”，关闭或超时只确认已提醒，实际进入优惠页并加载后才确认已查看；红点沿用此规则。

相同券/批次和离线确认继续去重；切换账号隔离收据，恢复连接后提交原确认。按服务端时间及单调时钟移除到期内容。关闭 App 后不新增系统推送。

删除是终态：券/商品从经营列表消失，不能再编辑、启用、发布或通过旧报价购买。删除商品与下单锁定同一商品记录并校验版本。底层保留订单引用、库存行、交付快照、提醒/发布批次及审计资料；已有订单的领取或退款照常进行，库存恢复不会使商品重新上架。券删除仅允许草稿或已停用状态，拒绝启用券删除时返回 `COUPON_DISABLE_REQUIRED`。删除响应为 `data: { couponId/productId, deleted: true, version }`，同一请求键重试返回原结果。
...
创建订单只引用原报价 ID/版本，重新检查商品发布版本、库存、配额、券版本/资格/有效期/占用和交付能力。券变更时要求重新确认，不能静默增加扣款或换券。对 `(couponId, owner_uuid)` 的唯一占用和订单建立处于同一事务。技术性失败释放未使用券，结果未知继续占用；成功后券持续占用，直到订单全额退款完成才恢复原券资格；退款失败、部分退款及结果未知均不释放。实付为零的订单同样在邮件撤回完成后返券。返券不改变原有效期、受众、启停和批次提醒记录。重复旧退款按原订单 ID 匹配，不能释放后来订单的新占用。
...
### 退款资格修复迁移（2026-09-11，本机待发布）

新增 `026_refunded_coupon_release.sql`：仅对 `OFFICIAL_STORE`、订单/资金均 `REFUNDED`、无待处理操作、零结算且退款金额等于原实付的历史订单移除券占用。重复执行安全；其余占用与全部订单/快照/提醒/批次记录保留。部署前备份；回退不得把已退回或再次使用的券占用覆盖成旧状态，也不得把 `DELETED` 的商品或券重新启用。旧版 Go 不认识删除终态，生产回退应使用保留终态保护的兼容版本。
