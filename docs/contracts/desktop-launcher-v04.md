...
适用：2026-09-12 的 Go / Web 2.0.13 启动器管理功能。Android App 沿用 2.0.12。桌面端固定 Minecraft 1.21.1、NeoForge 21.1.248；账户认证继续由桌面端直接使用微软正版链路。

## 公共读取

| 路径 | 返回 |
| --- | --- |
| `GET /api/v1/launcher/bootstrap` | 标准 v2 envelope 的 `data`：安装版本、初始基线、McPatch URL、带 size/SHA-256 的官方 Java 与 NeoForge 安装器信息；未发布返回 503 |
...
bootstrap 包含 `instanceVersion`、`minecraftVersion`、`loaderVersion`、`baselineVersion`、`mcpatchUrl`、`enabled`、`java`、`installer`。初次安装顺序为官方/镜像基础游戏、指定基线、再次检查并应用最新资源。
...
内容保留 arknights、endfield、popucom 三个稳定入口。静态图、视频、宣传图和公告正文配图上传后加入 `mediaAssets` 的 size/SHA-256 元数据；客户端缓存校验完成后切换，失败保留原缓存。
