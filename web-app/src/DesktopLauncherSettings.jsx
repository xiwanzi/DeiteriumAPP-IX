...
export default function DesktopLauncherSettings({ client }) {
  const [data, setData] = useState(null), [draft, setDraft] = useState(null), [entryIndex, setEntryIndex] = useState(1),
  ...
  return <section className="desktop-launcher-admin">
    ...
    {!draft?.content?.entries ? <p className="dl-loading">{data ? "整包版本尚未配置。" : "正在读取启动器配置…"}</p> : <>
      ...
      <fieldset disabled={busy} className="dl-editor-fieldset">
        ...
        </> : <div className="panel dl-form-card dl-install-settings"><h3>整包发布</h3><p className="dl-note">发布包需要包含完整游戏、Java 和资源版本标记。启动器只检查并安装 McPatch 后续更新。</p><Field label="客户端实例" readOnly value={draft.bootstrap.instanceVersion} /><Field label="整包对应的 McPatch 版本"><select value={draft.bootstrap.baselineVersion} onChange={(e) => mutate((next) => { next.bootstrap.baselineVersion=e.target.value; })}>{[...new Set([draft.bootstrap.baselineVersion, ...versions.map((v) => v.label)])].map((label) => <option key={label}>{label}</option>)}</select></Field><div className="dl-install-receipt"><strong>发布步骤</strong><span>准备完整客户端与 Java</span><span>使用整包打包脚本写入所选版本标记</span><span>后续在 McPatch 打包更新，再同步到 OSS</span></div></div>}
