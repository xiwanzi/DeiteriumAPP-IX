...
export default function OfficialAdmin({ client, user, navigate, search = "" }) {
  const permissions = user.permissions || [], all = permissions.includes("platform.admin"),
    tabs = [...(all || permissions.includes("intervention.manage") ? ["平台介入"] : []), ...(all || permissions.includes("announcements.manage") ? ["公告管理"] : []), ...(all || permissions.includes("audit.read") ? ["管理审计"] : []), ...(all ? ["小祥设置", "账号与权限", "邮件提醒"] : []), "商店管理", ...(all || permissions.includes("core.read") ? ["Core 管理"] : [])],
    requested = {interventions:"平台介入",announcements:"公告管理",audit:"管理审计",email:"邮件提醒",ai:"小祥设置",accounts:"账号与权限",core:"Core 管理"}[new URLSearchParams(search).get("section")],
