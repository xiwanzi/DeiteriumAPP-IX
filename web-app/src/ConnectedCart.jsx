...
export default function ConnectedCart({ client, user, navigate }) {
  ...
  return <><PageHead eyebrow="SHOPPING BAG" title="购物袋" subtitle="商品价格和库存将在结算时再次确认。"><Button secondary onClick={load} disabled={busy}>刷新</Button></PageHead>
    ...
    <div className="foundation-grid">{rows.map((item) => { const product = products[item.productId]; return <div className="foundation-card" key={item.productId}>
      <h3>{product?.content?.title || "商品暂不可用"}</h3>{product && <p className="price">{credit(product.content.price)}<small>信用点 / 件</small></p>}
      <Field label="数量"><select value={item.quantity} disabled={busy || !product} onChange={(e) => update(item, Number(e.target.value))}>{Array.from({ length: Math.max(item.quantity, Math.min(999, product?.content?.limitPerOrder || 1, product?.availableStock ?? 999)) }, (_, index) => <option key={index + 1} value={index + 1}>{index + 1}</option>)}</select></Field>
    ...
    {checkout && <Modal title="商城结算" close={() => setCheckout(false)}><PurchaseFlow client={client} user={user} channel="OFFICIAL_STORE" items={rows.map((item) => ({ productId: item.productId, quantity: item.quantity, expectedProductVersion: products[item.productId].version }))} onResource={(resource) => navigate(`/orders?order=${encodeURIComponent(resourceId(resource.value))}`)} /></Modal>}
  </>;
}
