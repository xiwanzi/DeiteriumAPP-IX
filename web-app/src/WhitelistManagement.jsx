...
function WhitelistEditor({ client, editor, close, onSaved }) {
  ...
  const save = async event => {
    ...
    try {
      const result = await client.request(path, { method: "POST", body: request.current.body });
      ...
      const mail = type === "review" ? result.data?.emailStatus === "PENDING" ? "审核结果邮件已排队。" : result.data?.emailStatus === "SKIPPED" ? "邮件提醒未开启，本次未发送邮件。" : "" : "";
      await onSaved(message + mail);
    } catch (e) { if (mounted.current) setError(e.message); } finally { if (mounted.current) setBusy(false); }
  };
  return <Modal title={title} close={() => { if (!busy) close(); }} dismissOnBackdrop={false}><form className="settings-stack" onSubmit={save}><fieldset disabled={busy || readOnly} className="whitelist-fieldset">
    ...
    {type === "review" && !readOnly && <><Field label="审批结果"><select value={decision} onChange={event => setDecision(event.target.value)}><option value="APPROVE">通过申请</option><option value="REJECT">拒绝申请</option></select></Field><p className="muted">启用审核邮件时，本次结果将发送至 {item.qq}@qq.com。</p></>}
