...
网站、原生 Android App、Go 后端与 Deuterium Core 的源码和运维入口。当前发布：2026-09-09 App/Web 2.0.4、配套 Go、Core 1.0.1 与 XConomy .2 已上线；Mail/Sync 保持既有版本。本轮源码已获用户授权公开推送。

| 组件 | 源码位置 | 当前发布基准 |
| --- | --- | --- |
| Android App | [android-app/ui-lab](android-app/ui-lab/README.md) | 2.0.4（20401），源码 `812f1d1a5056` |
...
现有服务入口为 [Deuterium IX](https://47.103.99.34)，App 下载及摘要见 [2.0.4 发布记录](docs/deployment/deuterium-2.0.4-live.md)。Android 包名仍为 `com.deuterium.app.uilab`；目录名 ui-lab 不代表当前仍是演示版。SMTP 配置页已上线，管理员填写实际邮箱参数后启用提醒。
