...
public class CommandPay extends CommandCore{
    public static boolean onCommand(CSender sender, String commandName, String[] args) {
        ...
        try {
            me.yic.xconomy.deuterium.ControlledEconomyAPI.nativePay(sender.toPlayer().getUniqueId(), targetUUID, taxamount, amount, com);
        } catch (me.yic.xconomy.deuterium.FundsFailure failed) {
            sendMessages(sender, PREFIX + failed.getMessage());
            return true;
        }
        ...
        // Both player balances and their ledger legs were committed above.
