...
def main():
    ...
    import re
    plugin,count=re.subn(r'(?m)^version:.*$',"version: '2.26.3-deuterium.1'",plugin)
    if count!=1:raise SystemExit('Unexpected plugin metadata')
    entries['plugin.yml']=plugin.encode();entries.update(modified)
    entries['META-INF/deuterium-xconomy-patch.json']=json.dumps({'version':'2.26.3-deuterium.1','inputSha256':EXPECTED,'apiVersion':1,'modifiedClasses':sorted(modified)},sort_keys=True,indent=2).encode()
